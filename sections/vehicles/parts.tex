...
Exactly one of each part is required. Each part contributes to one or two stats of your vehicle: add together the numbers for each stat from each part. For example, to find your vehicle's \stat{Acceleration}, add the values from the Engine and Aspiration rows. A vehicle's \stat{Max HP} is 5 times its \stat{Ruggedness}. When creating a vehicle for a new character, it can cost at most 500 scrap.
...
\begin{describe}{Nitrous boost}
  ...
  A single-use tank of N\textsubscript{2}O. It can be used in an \duration{\durationInstant{}} man\oe{}uvre to immediately increase your \stat{Speed} by 40.
\end{describe}
