...
\begin{describe}{Boarding}
  If you are on the outside of a vehicle, e.g. on its roof or flatbed, you can attempt to board another vehicle in the convoy. Roll \stat{\statSkill} vs the target's \stat{Handling}. If you succeed, you get on top of the target vehicle. If you fail, you fall off, but you can spend an action to return to your vehicle.
\end{describe}
...
\begin{describe}{Sabotage}
  If you are on the outside of a vehicle, you can attempt to severely damage it. Roll \stat{\statMental} vs the vehicle's \stat{Ruggedness}. If you succeed, you cause one affliction of your choice to the vehicle. The difficulty of the affliction must be less than half the difference between the rolls.
\end{describe}

\hr

\begin{describe}{Repair}
  As the last action in a setpiece, you can restore hit points to a vehicle or character. Roll \stat{\statMental} and add the target's \stat{\statPhysical} or \stat{Ruggedness}. Add the result to the target's HP. If the result is below zero, you'll end up doing damage, so be careful repairing something if you have negative \stat{\statMental}.
\end{describe}

\begin{describe}{Remove Affliction}
  As the last action in a setpiece, you can remove an affliction from a vehicle or character. Roll \stat{\statMental} and add the target's \stat{\statPhysical} or \stat{Ruggedness}. If the result is higher than the affliction's \stat{Difficulty} roll, remove the affliction.
\end{describe}
